<template>
      ...
      <div class="text-center max-w-md">
        ...
        <p class="text-neutral-400 dark:text-neutral-500 mb-6">
          ...
          <code
            ...
            >media</code
          >
          settings are wrong or you may need to create the
          <code
            class="text-sm bg-neutral-100 dark:bg-neutral-850 rounded-lg p-1"
            >{{ root }}</code
          >
        ...
        <div class="flex gap-x-2 justify-center">
          <router-link class="btn-primary" :to="{ name: 'settings' }"
            >Review settings</router-link
          >
        </div>
      </div>
    </div>
  ...
  <template v-else>
    <div class="fb">
      <header class="fb-header flex mb-4 gap-x-2">
        ...
        <ol v-if="breadcrumb" class="fb-breadcrumb flex items-center">
          <li v-if="path != root">
            <button
              @click="goTo(root)"
              class="fb-parent-link group relative btn-icon-sm !rounded-r-none"
            >
          ...
          <template v-for="(segment, index) in breadcrumb" :key="index">
            <li class="fb-breadcrumb-segment fb-breadcrumb-link">
              <template v-if="index < breadcrumb.length - 1">
                <button
                  @click="goTo(segment.path)"
                  class="btn-sm !rounded-none !border-l-0"
                >
                  {{ segment.label }}
                </button>
              </template>
              <template v-else>
                <div
                  class="fb-breadcrumb-segment fb-breadcrumb-leaf btn-sm !cursor-default !text-neutral-400 dark:!text-neutral-500 !font-normal !rounded-l-none !border-l-0"
                >
                  {{ segment.label }}
                </div>
              </template>
        ...
        <button
          ...
          class="fb-parent-link group relative btn-icon-sm"
        >
        ...
        <!-- Add a folder -->
        <button
          class="btn-icon-sm relative group ml-auto"
          @click="openAddFolderModal()"
        >
          <Icon name="FolderPlus" class="h-4 w-4 stroke-2 shrink-0" />
          <div
            class="spinner-white-sm"
            v-if="status == 'creating-folder'"
          ></div>
        ...
        <div class="fb-view flex">
          <button
            ...
            class="fb-view-list group btn-icon-sm !rounded-r-none relative"
            :disabled="layout == 'list'"
            :class="{
              '!bg-neutral-200 dark:!bg-neutral-700': layout == 'list',
            }"
          >
          ...
          <button
            ...
            class="fb-view-grid group btn-icon-sm !rounded-l-none !border-l-0 relative"
            :disabled="layout == 'grid'"
            :class="{
              '!bg-neutral-200 dark:!bg-neutral-700': layout == 'grid',
            }"
          >
      ...
      <!-- Content -->
      <div
        class="fb-files-wrapper"
        :class="[
          status == 'processing' ? 'processing' : '',
          isDragging ? 'fb-files-wrapper-dragging' : '',
        ]"
      ...
      >
        <ul
          ...
          class="fb-files"
          :class="[layout == 'grid' ? 'fb-files-grid' : 'fb-files-list']"
        >
          <li
            v-for="item in filteredContents"
            :key="item.path"
            class="fb-files-item"
            :class="[`fb-files-item-${item.type}`]"
          >
            <!-- Folders -->
            <template v-if="item.type == 'dir'">
              <button
                @click="goTo(item.path)"
                class="fb-files-item-content fb-files-item-link"
              >
                  ...
                  <Dropdown :dropdownClass="'!max-w-none w-48'">
                    <template #trigger>
                      <button class="fb-files-item-options-btn">
                        <Icon
                          name="MoreHorizontal"
                          class="h-4 w-4 stroke-2 shrink-0"
                        />
                      </button>
                    </template>
                    <template #content>
                      <ul>
                        <li>
                          <a
                            class="link w-full"
                            :href="`https://github.com/${props.owner}/${props.repo}/blob/${props.branch}/${item.path}`"
                            target="_blank"
                          >
                        ...
                        <li>
                          <hr
                            class="border-t border-neutral-150 dark:border-neutral-750 my-2"
                          />
                        </li>
                        <li>
                          <button
        ...
        <div
          v-else
          class="text-center rounded-xl bg-neutral-100 dark:bg-neutral-850 p-6"
        >
          <div class="max-w-md mx-auto">
            ...
            <p class="text-neutral-400 dark:text-neutral-500">
              <template v-if="filteredExtensions.length > 0"
                >There are no maching files in this folder ({{
                  filteredExtensions.join(", ")
                }})</template
              ><template v-else>There are no files in this folder</template>.
              You can drag-and-drop files here or use the "Upload file" button
              to add files.
            </p>
          </div>
        </div>
...
const handleDeleted = () => {
  const index = contents.value.findIndex(
    (item) => item.path === deletePath.value
  );
...
const selectToggle = (item) => {
  ...
  if (isSelected) {
    ...
    if (props.selectMax === 1) {
      ...
      props.selectMax !== undefined &&
      props.selectMax !== null &&
      currentSelection.length >= props.selectMax
    ) {
...
const breadcrumb = computed(() => {
  ...
  const breadcrumbSegments = pathSegments.map((segment, index) => {
    const fullPath =
      props.root + "/" + pathSegments.slice(0, index + 1).join("/");
    return {
      label: segment,
      path: fullPath,
    };
...
const setContents = async () => {
  ...
  try {
    ...
    contentsData = await github.getContents(
      props.owner,
      props.repo,
      props.branch,
      path.value || "",
    ...
    if (contentsData) {
      ...
      const fullPath = `${props.owner}/${props.repo}/${props.branch}/${
        path.value || ""
      }`;
      ...
      contents.value = contentsData.map((item) => {
        ...
        if (item.type === "file") {
          ...
          if (parts.length > 1 && parts[0] !== "") {
            ...
            kind =
              Object.keys(extensionCategories).find((key) =>
                extensionCategories[key].includes(extension)
              ) || "other";
          }
        }
