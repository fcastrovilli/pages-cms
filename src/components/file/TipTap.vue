  ...
  editorProps: {
    handleDrop: async function(view, event, slice, moved) {
      if (!moved && event.dataTransfer && event.dataTransfer.files && event.dataTransfer.files.length) {
        ...
        const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/svg+xml', 'image/bmp', 'image/tif', 'image/tiff', 'image/webp'];
        ...
        for (const file of files) {
          if (allowedTypes.includes(file.type)) {
            ...
            notifications.notify('Only images can be uploaded (JPEG, PNG, GIF, SVG, TIFF, BMP and WebP).', 'error');
          }
        }
        return true;
      }
